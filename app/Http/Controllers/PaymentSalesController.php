...
class PaymentSalesController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        // Auto Notification
        try {
            $payment_sale = PaymentSale::latest()->first();
            if ($payment_sale) {
                $this->Auto_Notification($payment_sale->id);
            }
        } catch (\Exception $e) {
            // Log or ignore notification errors to not block the main process
        }
    ...
    /**
     * Send Auto Notification (SMS & Email)
     */
    private function Auto_Notification($payment_id)
    {
        $payment = PaymentSale::with('sale.client')->findOrFail($payment_id);
        ...
        if ($emailMessage && $payment['sale']['client']->email) {
            $receiver_email = $payment['sale']['client']->email;
            $message_body = str_replace(['{contact_name}', '{business_name}', '{payment_number}', '{total_amount}'], 
                                        [$contact_name, $business_name, $payment_number, $total_amount], 
                                        $emailMessage->body);
            ...
            try {
                ...
                Mail::to($receiver_email)->send(new CustomEmail($email_data));
            } catch (\Exception $e) {
                // Ignore email errors
            }
        }
        ...
        // --- Send SMS ---
        $default_sms_gateway = sms_gateway::where('id' , $settings->sms_gateway)->where('deleted_at', '=', null)->first();
        $smsMessage  = SMSMessage::where('name', 'payment_received')->first();
        if ($default_sms_gateway && $smsMessage && $payment['sale']['client']->phone) {
            $receiverNumber = $payment['sale']['client']->phone;
            $message_text = str_replace(['{contact_name}', '{business_name}', '{payment_number}', '{total_amount}'], 
                                        [$contact_name, $business_name, $payment_number, $total_amount], 
                                        $smsMessage->text);
            ...
            if ($default_sms_gateway->title == "twilio") {
                try {
                    ...
                    $client->messages->create($receiverNumber, ['from' => env("TWILIO_FROM"), 'body' => $message_text]);
                } catch (\Exception $e) {}
                ...
                try {
                    ...
                    $sendSmsApi = new SendSMSApi(new Client_guzzle(), $configuration);
