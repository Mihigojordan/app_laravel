      ...
      >
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <a v-if="props.row.can_approve" @click="Approve_Payroll(props.row.id)" class="cursor-pointer" title="Approve" v-b-tooltip.hover>
              <i class="i-Checked-User text-25 text-primary"></i>
            </a>
            <a v-if="props.row.can_approve" @click="Reject_Payroll(props.row.id)" class="cursor-pointer" title="Reject" v-b-tooltip.hover>
              <i class="i-Close text-25 text-warning"></i>
            </a>
   ...
   //------------------------------ Show Modal (Create Payroll) -------------------------------\\
      ...
      this.$swal({
        ...
        if (result.value) {
          axios
            ...
            .then(() => {
              this.makeToast("success", "Payroll approved successfully", "Success");
              Fire.$emit("Event_Payroll");
            })
            .catch(error => {
              this.makeToast("danger", error.response.data.message || "Failed", "Failed");
            });
        }
      });
    },
    ...
    Reject_Payroll(id) {
      this.$swal({
        ...
        if (result.value) {
          axios
            .post("payroll/" + id + "/reject", { comment: result.value })
            .then(() => {
              this.makeToast("success", "Payroll rejected", "Success");
              Fire.$emit("Event_Payroll");
            })
            .catch(error => {
              this.makeToast("danger", error.response.data.message || "Failed", "Failed");
