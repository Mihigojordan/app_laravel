<template>
  <div class="main-content">
    <breadcumb :page="$t('EditRequisition')" :folder="$t('Procurement')"/>
    <div v-if="isLoading" class="loading_page spinner spinner-primary mr-3"></div>

    <validation-observer ref="edit_requisition" v-if="!isLoading">
...
export default {
  ...
  methods: {
    Get_Elements() {
      ...
      axios
        .get("requisitions/" + id + "/edit")
        .then(response => {
          this.requisition = response.data.requisition;
          this.details = response.data.details;
          this.warehouses = response.data.warehouses;
          this.Get_Products_By_Warehouse(this.requisition.warehouse_id);
    ...
    Get_Products_By_Warehouse(id) {
      ...
      axios
        ...
        .then(response => {
          ...
          // Update current stock in details
          this.details.forEach(detail => {
              const prod = this.products.find(p => p.id == detail.product_id);
              if (prod) {
                  detail.stock = prod.qte;
              }
          });
          NProgress.done();
        })
    ...
    Submit_Requisition() {
      this.$refs.edit_requisition.validate().then(success => {
        if (!success) {
          this.makeToast(
            "danger",
